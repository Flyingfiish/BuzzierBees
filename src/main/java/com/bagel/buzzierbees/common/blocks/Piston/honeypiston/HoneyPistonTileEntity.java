...
public class HoneyPistonTileEntity extends TileEntity implements ITickableTileEntity {
   ...
   public HoneyPistonTileEntity() {
      super(ModTileEntities.HONEY_PISTON.get());
   }

   public HoneyPistonTileEntity(BlockState pistonStateIn, Direction pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
      this();
   ...
   private BlockState getCollisionRelatedBlockState() {
      return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof PistonBlock ? ModBlocks.PISTON_HEAD.getDefaultState().with(HoneyPistonHeadBlock.TYPE, this.pistonState.getBlock() == ModBlocks.HONEY_PISTON ? PistonType.STICKY : PistonType.DEFAULT).with(HoneyPistonHeadBlock.FACING, this.pistonState.get(PistonBlock.FACING)) : this.pistonState;
   }
   ...
   public void clearPistonTileEntity() {
      if (this.lastProgress < 1.0F && this.world != null) {
         ...
         this.remove();
         if (this.world.getBlockState(this.pos).getBlock() == ModBlocks.MOVING_HONEY_PISTON) {
   ...
   public void tick() {
      ...
      if (this.lastProgress >= 1.0F) {
         this.world.removeTileEntity(this.pos);
         this.remove();
         if (this.pistonState != null && this.world.getBlockState(this.pos).getBlock() == ModBlocks.MOVING_HONEY_PISTON) {
   ...
   public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
      ...
      if (!this.extending && this.shouldHeadBeRendered) {
         voxelshape = this.pistonState.with(HoneyPistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMotionDirection()) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            blockstate = ModBlocks.HONEY_PISTON_HEAD.getDefaultState().with(HoneyPistonHeadBlock.FACING, this.pistonFacing).with(HoneyPistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
         } else {
