...
public class AdvancedPistonTileEntity extends TileEntity implements ITickableTileEntity {
   ...
   public void clearPistonTileEntity() {
      if (this.lastProgress < 1.0F && this.world != null) {
         ...
         this.remove();
         if (this.world.getBlockState(this.pos).getBlock() == ModBlocks.MOVING_PISTON) {
   ...
   public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
      ...
      if (!this.extending && this.shouldHeadBeRendered) {
         voxelshape = this.pistonState.with(AdvancedPistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMotionDirection()) {
         ...
         if (this.shouldPistonHeadBeRendered()) {
            blockstate = ModBlocks.PISTON_HEAD.getDefaultState().with(AdvancedPistonHeadBlock.FACING, this.pistonFacing).with(AdvancedPistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 4.0F));
         } else {
